...
-- Implementar nuevos roles de seguridad: Supervisor, Analista, Usuario (sin acceso)
...
-- PASO 4: Crear permiso especial para verificar acceso al sistema
INSERT INTO permissions (name, description, resource, action) VALUES
('system.access', 'Acceso básico al sistema', 'system', 'access')
ON CONFLICT (name) DO NOTHING;

-- PASO 5: Asignar permisos al rol SUPERVISOR
...
AND p.name IN (
    ...
    -- Control de casos: acceso completo excepto eliminación
    'case_control.view',
...
-- PASO 6: Asignar permisos al rol ANALISTA
-- El analista solo puede trabajar con sus propios casos, sin eliminar
INSERT INTO role_permissions (role_id, permission_id)
SELECT 
    r.id as role_id,
    p.id as permission_id
...
AND p.name IN (
    -- Casos: solo propios, sin eliminar
    'cases.create',
    'cases.read.own',
    'cases.update.own',
    
    -- Orígenes: solo lectura
    'origenes.read',
    
    -- Aplicaciones: solo lectura
    'aplicaciones.read',
    
    -- Control de casos: solo sus propios casos
    'case_control.view',
...
-- PASO 7: Asignar permiso de acceso al sistema a admin (si no lo tiene)
INSERT INTO role_permissions (role_id, permission_id)
SELECT 
    r.id as role_id,
    p.id as permission_id
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'has_system_access') THEN
        RAISE NOTICE 'Función has_system_access creada correctamente';
    ELSE
        RAISE EXCEPTION 'Error: Función has_system_access no se creó';
    END IF;
END $$;
